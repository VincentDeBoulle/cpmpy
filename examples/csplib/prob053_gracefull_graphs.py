...
if __name__ == "__main__":
    ...
    tablesp_ortools =  PrettyTable(['Model Creation Time', 'Solver Creation + Transform Time', 'Solve Time', 'Overall Execution Time', 'Overall Memory Usage (Bytes)'])
    tablesp_ortools.title = 'Results of the gracefull graphs problem without CSE'
    tablesp_ortools_CSE =  PrettyTable(['Model Creation Time', 'Solver Creation + Transform Time', 'Solve Time', 'Overall Execution Time', 'Overall Memory Usage (Bytes)'])
    tablesp_ortools_CSE.title = 'Results of the gracefull graphs problem with CSE' 
    tablesp_ortools_factor =  PrettyTable(['Model Creation Time', 'Solver Creation + Transform Time', 'Solve Time', 'Overall Execution Time', 'Overall Memory Usage (Bytes)'])
    ...
    for slvr in ["z3", "z3_2"]:
            ...
            for lp in range(nb_iterations):
                ...
                (_, transform_time, solve_time), model_creation_time = run_code(slvr)
            ...
            if slvr == 'z3':
                ...
                tablesp_ortools.add_row([average_model_creation_time, average_transform_time, average_solve_time, average_execution_time, average_mem_usage])
                with open("cpmpy/timing_results/gracefull_graphs_z3.txt", "w") as f:
                ...
                tablesp_ortools_CSE.add_row([average_model_creation_time_2, average_transform_time_2, average_solve_time_2, average_execution_time_2, average_mem_usage_2])
                with open("cpmpy/timing_results/gracefull_graphs_z3_CSE.txt", "w") as f:
                ...
                tablesp_ortools_factor.add_row([factor_model_creation_time, factor_tranform_time, factor_solve_time, factor_execution_time, factor_mem_usage])
                with open("cpmpy/CSE_results/gracefull_graphs_z3.txt", "w") as f:

...
if __name__ == "__main__":
    ...
    tablesp_ortools = PrettyTable(['Board size', 'Model Creation Time', 'Solver Creation + Transform Time', 'Solve Time', 'Overall Execution Time', 'Overall Memory Usage (Bytes)'])
    tablesp_ortools.title = 'Results of the Peaceably Co-existing Armies of Queens problem without CSE'
    tablesp_ortools_CSE =  PrettyTable(['Board size', 'Model Creation Time', 'Solver Creation + Transform Time', 'Solve Time', 'Overall Execution Time', 'Overall Memory Usage (Bytes)'])
    tablesp_ortools_CSE.title = 'Results of the Peaceably Co-existing Armies of Queens problem with CSE'
    tablesp_ortools_factor =  PrettyTable(['Board size', 'Model Creation Time', 'Solver Creation + Transform Time', 'Solve Time', 'Overall Execution Time', 'Overall Memory Usage (Bytes)'])
    ...
    for sz in range(4, 9):
        ...
        for slvr in ['z3', 'z3_2']:
            ...
            for lp in range(nb_iterations):
                ...
                (n_sols, transform_time, solve_time), model_creation_time, black_queens, white_queens = run_code(slvr)
            ...
            if slvr == 'z3':
                ...
                tablesp_ortools.add_row([n, average_model_creation_time, average_transform_time, average_solve_time, average_execution_time, average_mem_usage])
                with open("cpmpy/timing_results/peaceably_queens_z3.txt", "w") as f:
            ...
            if slvr == 'z3_2':
                ...
                tablesp_ortools_CSE.add_row([n, average_model_creation_time_2, average_transform_time_2, average_solve_time_2, average_execution_time_2, average_mem_usage_2])
                with open("cpmpy/timing_results/peaceably_queens_z3_CSE.txt", "w") as f:
                ...
                tablesp_ortools_factor.add_row([n, factor_model_creation_time, factor_tranform_time, factor_solve_time, factor_execution_time, factor_mem_usage])
                with open("cpmpy/CSE_results/peaceably_queens_z3.txt", "w") as f:

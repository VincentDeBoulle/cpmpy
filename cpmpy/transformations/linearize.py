...
def linearize_constraint(lst_of_expr, supported={"sum","wsum"}, reified=False, expr_dict=None):
    ...
    for cpm_expr in lst_of_expr:
        ...
        # comparisons
            ...
            if cpm_expr.name == "<":
                new_rhs, cons = get_or_make_var(rhs - 1, expr_dict) # if rhs is constant, will return new constant
                newlist.append(lhs <= new_rhs)
                newlist += linearize_constraint(cons, expr_dict)
            elif cpm_expr.name == ">":
                new_rhs, cons = get_or_make_var(rhs + 1, expr_dict) # if rhs is constant, will return new constant
                newlist.append(lhs >= new_rhs)
                newlist += linearize_constraint(cons, expr_dict)
                ...
                if reified or (isinstance(lhs, (Operator, GlobalConstraint)) and lhs.name not in {"sum","wsum"}):
                    ...
                    newlist += linearize_constraint(flatten_constraint(cons, expr_dict), supported=supported, reified=reified, expr_dict=expr_dict)

        ...
        if cpm_expr.name in ['->', "=="]:
            ...
            if not isinstance(a0, _BoolVarImpl) and \
                ...
                if cpm_expr.name == '->':
                    ...
                    newexpr = only_bv_reifies(flatten_constraint(newexpr, expr_dict), expr_dict)
                else:
                    newexpr = [a1 == a0]  # BE == BV :: BV == BE
                    if not a0.is_bool():
                        newexpr = flatten_constraint(newexpr, expr_dict)
    ...
    for cpm_expr in constraints:
        ...
        if cpm_expr.name == '->':
            ...
            if isinstance(a1, Comparison) and \
                ...
                newcons.extend(only_implies(flatten_constraint(newexpr, expr_dict), expr_dict))
            else:
                newcons.append(cpm_expr)
        ...
        # Comparisons: transform bV == BE
            ...
            if isinstance(a0, _BoolVarImpl) and isinstance(a1, _BoolVarImpl):
                ...
                if isinstance(a1, GlobalConstraint):
                    ...
                    newcons.extend(only_implies(only_bv_reifies(flatten_constraint(newexprs, expr_dict), expr_dict), expr_dict))
        else:
        ...
        if boolexpr_index is None:  # non-reified or variable-only reification
            ...
            if isinstance(boolexpr, Operator):
                ...
                if isinstance(lhs, _NumVarImpl) or lhs.name in supported:
                    ...
                    newcons += flatten_constraint(reifexpr, expr_dict)
                    ...
                    (auxvar, cons) = get_or_make_var(lhs, expr_dict)
                    newcons += cons
                    reifexpr = copy.copy(cpm_expr)
